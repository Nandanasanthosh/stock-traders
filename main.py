...
newsapi_token = config.newsapi_token
...
@bot.command(name="stock",help="Return current value and change")
...
async def chart(ctx, quo="msft"):
    ...
    data1 = si.get_data(quo, old_date, current_date)
    # clear the graph before creating one
    plt.clf()

    # plot the graph and store it in a variable
    plot = data1.plot(title='Stock Price over 2 years')
    fig = plot.get_figure()
    filename = 'output.png'
